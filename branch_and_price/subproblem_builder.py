...
class SubproblemBuilder:
    ...
    def build(
        ...
        task_to_variable = self._build_columns(
            model, machine_id, task_duals, branching_rules
        )

        self._build_capacity_constraint(model, machine_id, task_to_variable)
    ...
    def _build_capacity_constraint(
        self, model: grb.Model, machine_id: int, task_to_variable: Dict[int, grb.Var]
    ):
        lhs = grb.quicksum(
            [
                self._gap_instance.weight(task_id, machine_id)
                * task_to_variable[task_id]
    ...
    def _build_columns(
        self, model, machine_id, task_duals, branching_rules
    ) -> Dict[int, grb.Var]:
        ...
        for task_id in range(self._gap_instance.num_tasks):
            ...
            obj = (
                self._gap_instance.assignment_profit(
                    task_id=task_id, machine_id=machine_id
                )
                - task_duals[task_id]
            )
            var = model.addVar(
    ...
    @classmethod
    def _lower_and_upper_bound(
        cls, machine: int, task: int, branching_rules: List[BranchingRule]
    ) -> Tuple[float, float]:
        """
