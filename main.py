...
def main():
    try:
        ...
        fmt_basic = "%(message)s"
        logging.basicConfig(
            format=fmt_basic, datefmt="%Y/%m/%d %I:%M:%S %p", level=logging.INFO
        )

        parser = argparse.ArgumentParser(
            description="Solves machine assignment problem."
        )
        parser.add_argument(
            "data_set",
            choices=[
        ...
        if use_standalone_model:
            ...
            lp_relaxation = GAPStandaloneModelLpRelaxation(
                mip_model=gap_model.mip_model
            )
            lp_relaxation.solve()
            lp_relaxation.report_results()

            dw_gap_model = DantzigWolfeFormulationGapStandaloneModelBuilder(
                gap_instance=gap
            ).build()
            dw_gap_model.write()
            dw_gap_model.solve()
            ...
            dw_lp_relaxation = DantzigWolfeFormulationGapStandaloneModelLpRelaxation(
                mip_model=dw_gap_model.dw_model
            )
